...
def reindex_material_layer_nodes(material_channel_name, context):
...
def relink_layers(material_channel_name, context):
    '''Re-links all layers in the specified material channel together.'''
    layers = context.scene.matlay_layers
    material_channel_node = material_channels.get_material_channel_node(context, material_channel_name)
...
def relink_all_layer_nodes():
    '''Cycles through all nodes within the material node tree for the active material and re-links everything.'''
...
def update_layer_nodes(context):
    ...
    material_channel_names = material_channels.get_material_channel_list()
    for material_channel in material_channel_names:
        reindex_material_layer_nodes(material_channel, context)
        organize_layer_nodes(material_channel, context)
